...
- **At-a-glance activity:** prefer one mutable status item with `state · worker · current meaningful action`. Progress edits are coalesced to at most one every 2 seconds and rendered within Discord message limits. Do not turn tool calls into a scrolling transcript. Silence, timeout, waiting, blocked, and failed are explicit states.
