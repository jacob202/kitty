...
- Base exact fresh GitHub main `af3f6323d47be79c7f4d890d60ba54a99282d70f`.
...
- [ ] Write tests proving idempotency-key request conflicts fail, normal success is cached, and commit-then-response-loss is reconciled after reopening SQLite without a second effect.
- [ ] Run `python -m pytest tests/test_builder_operability.py -q` and verify RED because module/table/API is absent.
- [ ] Add the table and minimal implementation. `unknown` must invoke the verifier before any retry; verifier states are `applied`, `not_applied`, `unknown`.
- [ ] Run the focused test and verify GREEN.
- [ ] Commit `feat(builder): add durable operable invocation receipts`.
...
- [ ] Write tests for legacy single-step compilation, multi-step artifact/control transfer, unknown successor, duplicate IDs, missing artifact producer, and cycles/unreachable steps.
- [ ] Run the focused tests and verify RED on missing `compile_workflow` behavior.
- [ ] Implement deterministic normalization/validation; do not invoke an LLM.
- [ ] Run focused tests and verify GREEN.
- [ ] Commit `feat(builder): compile contracts into explicit workflows`.
...
- [ ] Write tests proving cheap/free→frontier uses artifact-compacted handoff, frontier→cheap/free preserves trajectory, same-tier continues, task classes select the four profiles, and malformed fallback candidates fail closed.
- [ ] Run focused tests and verify RED.
- [ ] Implement the minimal deterministic policy and optional candidate parsing. No executor fallback is introduced in the concurrently owned loop.
- [ ] Run focused tests and verify GREEN.
- [ ] Commit `feat(builder): make model handoffs and harness policy explicit`.
...
- [ ] Write migration/round-trip tests on a pre-column DB and verify duplicate-work protection remains.
- [ ] Run focused tests and verify RED.
- [ ] Add idempotent column migration and canonical JSON serialization; reject non-serializable policy rather than dropping it.
- [ ] Run focused tests and verify GREEN.
- [ ] Commit `feat(builder): persist model routing policy receipts`.
...
- [ ] Write tests for exact matched pairs, task-set mismatch failure, minimum-lift decision, no signal for non-improvement, and two distinct sessions promoting the same positive lesson.
- [ ] Run focused tests and verify RED.
- [ ] Implement comparison and translation into the existing workflow-signal store; no skill file is auto-edited.
- [ ] Run focused tests and verify GREEN.
- [ ] Commit `feat(builder): measure and distill capability improvements`.
...
- [ ] Run `git diff --check`.
- [ ] Run the exact combined focused suite from AC-6.
- [ ] Run narrow lint/type checks for changed Python files if available without widening into unrelated failures.
- [ ] Confirm forbidden overlapping files are unchanged relative to base.
- [ ] Attempt a genuinely separate free reviewer process with only spec, SHA/diff, tests, and evidence. If unavailable, classify as `implemented, awaiting verification` rather than `verified`.
