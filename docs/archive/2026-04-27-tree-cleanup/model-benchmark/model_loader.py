...
DEFAULT_MODEL = "mlx-community/DeepSeek-R1-0528-Qwen3-8B-4bit"

# Mapping of task types to preferred models (update after benchmark!)
# You will set these based on your test results.
TASK_MODEL_MAP: Dict[str, str] = {
    "conversation": "mlx-community/DeepSeek-R1-0528-Qwen3-8B-4bit",
    "automotive": "mlx-community/Qwen3-8B-4bit",   # adjust after test
    "fitness": "mlx-community/DeepSeek-R1-0528-Qwen3-8B-4bit",
    "growth": "mlx-community/DeepSeek-R1-0528-Qwen3-8B-4bit",
    "routing": "mlx-community/Orchestrator-8B-4bit",
    "code": "mlx-community/DeepSeek-R1-0528-Qwen3-8B-4bit",
...
class ModelSwitcher:
    ...
    def load_if_needed(self, task_type: str) -> tuple:
        ...
        if model_name != self.current_model:
            ...
            # Potentially offload previous model? MLX's memory management handles it automatically.
            return model_name, model, tokenizer
        else:
            print(f"Reusing cached {model_name}")
            # Reload if not already in local variable; for simplicity we'll just load again (fast)
            model, tokenizer = mlx_lm.load(model_name)
            return model_name, model, tokenizer
