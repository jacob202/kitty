...
- **A2 legacy retirement — done (2026-06-19).** Deleted `start_all.sh`, `stop_all.sh`, `status_all.sh`, `runtime_manifest.json`, `tests/test_gateway_runtime_paths.py`; archived `HANDOFF.md` and `gateway/ADVANCED_WORKFLOWS.md`. Test baseline: 491 passed, 2 deselected. Remaining optional cleanup: `start_openwebui.sh` (dead, OWU gone), `start_tool_servers.sh`, `run_doctor_check.sh` — kept for now since they don't break tests and may be reactivated.
