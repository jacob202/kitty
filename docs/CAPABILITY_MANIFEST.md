# Capability Manifest — Core Specification
...
- **Constitution v1 §III.5 (Honest State):** The manifest implements the five-state lattice. Every fact obeys the honesty rules. No fact is ever fabricated.
- **ADR 0029 (Capability Manifest Single Truth):** This specification is the implementation of ADR 0029. It supersedes the schema outline in KPA-01.
- **ADR 0032 (Evidence-Backed Claims):** The manifest's `evidence_ref` field implements the evidence requirement. Every claim about capability traces to a probed source.
- **KITTY_PRODUCT_ARCHITECTURE.md §4:** This specification supersedes the schema described there. The schema here is more detailed and includes sections the original document deferred (MCP, memory, image, shell).
- **docs/OPENWEBUI_PRODUCT_PLAN.md:** The product plan's Event Functions (`kitty_context_injector`, `kitty_shell_status`, `kitty_tool_auth`) consume this manifest. The manifest is the data contract between Gateway and shell.
- **gateway/runtime_manifest.py:** The existing v1 implementation. This specification defines v2. The existing code is the starting point for implementation.
