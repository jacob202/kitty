...
```bash
venv/bin/python -m pytest tests/test_web_chat_phase1.py tests/test_brief_route.py tests/test_commands_route.py -q --tb=short
```

Exit code: 127

```text
scripts/run_phase4_merge_gate.sh: line 96: venv/bin/python: No such file or directory
```
...
```text
{"confidence":0.85,"conversation_id":null,"fallback_used":false,"ok":true,"response":"Provider fallback failed. Anthropic: Error code: 401 - {'type': 'error', 'error': {'type': 'authentication_error', 'message': 'invalid x-api-key'}, 'request_id': 'req_011CaaPnpmawjoGsrcZYnW5B'}","safety_warnings":[],"sentiment":0.0,"sources":[],"specialist":"Kitty","suggested_followups":[]}
```
