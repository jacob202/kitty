# Handoff — KTF-001 recovery plan checkpoint
...
- Main is green after PR #295; PRs #261–#263 are merged and PR #293 is closed.
- `f9dfb6a` is an ancestor of current main and the Outcome 6 code anchors are
  present, but the original Builder records are cancelled/failed. That mismatch
  is evidence to reconcile, not permission to rerun stale packet instructions.
- The canonical Builder projection must be queried from the canonical checkout;
  an isolated worktree can expose an empty local Builder database.

## Resume exactly here

1. Run KTF-R1 from `docs/initiatives/ktf-001-resume-proof-v2.json`.
2. Write the named reliability-delta report from supported sources only.
3. Author only the fresh manifests that report proves are needed; validate and
   independently review them before applying anything.

## Do not do

- Do not claim KB-BRAIN, KX, B3, or B7 work while KTF-001 remains active.
- Do not replay `ktf-001-free-exec-v1`, `ktf-003-outcome6-runtime-v1`, or the
  daylight-exhaustion manifest: their durable task history is terminal and
  their literal anchors no longer describe current main.
