...
def resolve_base_sha(repo_root: Path | None = None) -> str:
    """Resolve an immutable full SHA-256 from the current main branch HEAD.
    ...
    Raises ``BaseSHAResolutionError`` if no branch ref can be resolved.
    """
    ...
    raise BaseSHAResolutionError(
        "cannot resolve base SHA: no valid main branch ref found"
    )
