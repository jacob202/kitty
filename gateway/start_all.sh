...
# Starts the Kitty stack: LiteLLM proxy (:8001), gateway (:5001), kitty-chat UI (:3000).
# Optional extras via env flags: ENABLE_MLX=1, ENABLE_CLOUDFLARE_HTTPS=1, ENABLE_UI=0.
...
service_pattern() {
  ...
  case "${name}" in
    ...
    # Port-agnostic so a custom GATEWAY_PORT still matches pgrep.
...
service_session_command() {
  ...
  case "${name}" in
    ...
    ui) echo "npm run dev --prefix gateway/kitty-chat" ;;
    *) echo "" ;;
  esac
}
...
if [[ "${START_ALL_SMOKE}" == "1" ]]; then
  ...
  [[ "${ENABLE_UI}" == "1" ]] && echo "UI smoke skipped (launcher is runtime-only)."
...
if [[ "${ENABLE_UI}" == "1" ]]; then
  start_service "ui" "cd '${ROOT_DIR}' && npm run dev --prefix gateway/kitty-chat"
fi
...
[[ "${ENABLE_GATEWAY}" == "1" ]] && wait_http "gateway" "http://127.0.0.1:${GATEWAY_PORT}/health" || true
[[ "${ENABLE_UI}" == "1" ]] && wait_http "ui" "http://127.0.0.1:3000" "" 45 2 4 || true

echo
echo "Stack launch complete."
echo "Kitty chat UI (Next.js):     http://127.0.0.1:3000"
echo "Kitty Gateway (FastAPI):     http://127.0.0.1:${GATEWAY_PORT}"
echo "LiteLLM proxy:               http://127.0.0.1:8001"
[[ "${ENABLE_MLX}" == "1" ]] && echo "MLX:                         http://127.0.0.1:8010"
echo
echo "Logs:"
