...
def list_loops() -> list[dict]:
    ...
    with sqlite3.connect(LOOPS_DB) as conn:
        ...
        rows = conn.execute(
            "SELECT * FROM loops ORDER BY created_at ASC"
        ).fetchall()
...
def _next_available_id(conn: sqlite3.Connection, base_id: str) -> str:
    ...
    if conn.execute(
        "SELECT 1 FROM loops WHERE loop_id = ?", (base_id,)
    ).fetchone() is None:
        return base_id
    n = 1
    while True:
        ...
        if conn.execute(
            "SELECT 1 FROM loops WHERE loop_id = ?", (candidate,)
        ).fetchone() is None:
            return candidate
        n += 1
...
def delete_loop(loop_id: str) -> bool:
    ...
    with sqlite3.connect(LOOPS_DB) as conn:
        cursor = conn.execute(
            "DELETE FROM loops WHERE loop_id = ?", (loop_id,)
        )
        conn.commit()
