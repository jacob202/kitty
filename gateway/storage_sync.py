...
def import_memories(payload: list[dict]) -> int:
    ...
    for record in payload:
        ...
        namespace = (
            ...
            or (record.get("metadata") or {}).get("namespace")
            or "facts"
        )
...
def import_journal_entries(payload: list[dict]) -> int:
    if not isinstance(payload, list):
        raise ValueError(
            f"journal_entries payload must be a list, got {type(payload).__name__}"
        )
    added = 0
    for record in payload:
        if not isinstance(record, dict):
...
def import_plugin_settings(payload: dict[str, bool]) -> int:
    if not isinstance(payload, dict):
        raise ValueError(
            f"plugin_settings payload must be a dict, got {type(payload).__name__}"
        )
...
def import_preferences(payload: dict) -> int:
    ...
    if not isinstance(payload, dict):
        raise ValueError(
            f"preferences payload must be a dict, got {type(payload).__name__}"
        )
    return len(payload)
