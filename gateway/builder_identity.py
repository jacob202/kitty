...
def _get_allowed_paths(
    ...
    """Return the valid durable allowlist for one initiative packet.
    ...
    if not rows:
        raise IdentityError(
            f"packet {initiative_id!r}/{packet_id!r} has no allowlist row"
        )
    if len(rows) != 1:
        raise IdentityError(
            f"packet {initiative_id!r}/{packet_id!r} has ambiguous allowlist rows"
        )
    try:
        ...
        raise IdentityError(
            f"packet {initiative_id!r}/{packet_id!r} has invalid allowed_paths_json: {exc}"
        ) from exc
