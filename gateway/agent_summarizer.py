...
class AgentSummarizer:
    ...
    async def _summarization_loop(self):
        ...
        while self._running:
            try:
                ...
                logger.error(f"Error in summarization loop for session {self.session_id}: {e}")
    ...
    async def _generate_summary(self):
        ...
        try:
            ...
            recent_messages = [
                h for h in history[-10:]  # Last 10 entries
            ...
            conversation_text = "\n\n".join([
                f"Iteration {i+1}: {msg['content'][:500]}" 
                for i, msg in enumerate(recent_messages[-3:])  # Last 3 assistant messages
            ])
            ...
            if summary != self._last_summary:
                ...
                logger.debug(f"Updated summary for session {self.session_id}: {summary[:100]}...")
...
def start_agent_summarizer(session_id: int,
                          get_agent_history: Callable[[], list[dict]],
                          update_summary: Callable[[str], None],
                          interval_seconds: int = 30) -> AgentSummarizer:
