                ...
                matches.append(
                    Item(
                        ...
                        metadata={
                            ...
                            if k not in {"user_request", "timestamp"}
                        },
                    )
                )
            ...
            if isinstance(res, Exception):
                if isinstance(res, TimeoutError):
                    ...
                    result.errors.append(
                        f"{adapter.name}: {type(res).__name__}: timed out"
                    )
                else:
                    logger.warning("%s fetch failed: %s", adapter.name, res)
                    result.errors.append(
                        f"{adapter.name}: {type(res).__name__}: {res}"
                    )
...
def _format_unified_items(
    results: dict[str, list[Item]], cap: int = CONTEXT_TOKEN_CAP
) -> str:
