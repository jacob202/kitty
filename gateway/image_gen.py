"""ComfyUI image generation — calls the local ComfyUI REST API."""
import asyncio
import os
import time
import uuid
...
COMFY_OUT  = Path(os.environ.get("COMFY_OUTPUT_DIR",
                  "/Users/jacobbrizinski/Projects/imagegen/ComfyUI/output"))
...
def _seed() -> int:
    return int.from_bytes(__import__("os").urandom(8), "little") & 0xFFFFFFFFFFFFFFFF
...
async def generate(prompt: str) -> dict:
    """Generate an image from a text prompt. Returns {filename, path, url}."""
    p = _parse(prompt)

    if p["sdxl"]:
        ckpt = SDXL_PHOTONIC if p["photonic"] else SDXL_PHOTONIC
        workflow = _wf_sdxl(prompt, p["negative"], p["w"], p["h"], p["steps"], p["cfg"], ckpt)
    else:
