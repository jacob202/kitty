...
for (const viewport of VIEWPORTS) {
  test(`PR #672 exact-head trust acceptance at ${viewport.width}x${viewport.height}`, async ({ page }, testInfo) => {
    test.skip(testInfo.project.name !== 'hermetic-chromium', 'requires isolated hermetic Kitty state')
    await page.setViewportSize({ width: viewport.width, height: viewport.height })
    ...
    let phase = 'healthy'
    page.on('pageerror', error => browserErrors.push(`${phase}:page:${String(error)}`))
    page.on('console', message => { if (message.type() === 'error') browserErrors.push(`${phase}:console:${message.text()}`) })
    await installHermeticStubs(page, providerDispatches, controlPlaneWrites, unexpectedPaths)

    const openChat = async () => {
      const button = page.getByRole('button', { name: /^chat$/i }).first()
      await button.click()
    ...
    await page.goto('/')
    await openChat()
    await expect(composer()).toBeEnabled({ timeout: 12_000 })
    await expect(page.getByRole('button', { name: 'Model: Daily Kitty' })).toBeEnabled()
    await assertNoHorizontalOverflow(page)
    await page.screenshot({ path: testInfo.outputPath('01-healthy.png'), fullPage: true })

    phase = 'picker-failure'
    await page.route('**/proxy/models/picker', route => json(route, { error: 'acceptance injected picker outage' }, 503))
    await page.reload()
    await openChat()
    await expect(composer()).toBeDisabled({ timeout: 12_000 })
    const status = page.getByRole('status').filter({ hasText: /Model details unavailable|Model details timed out|No live curated models/i }).first()
    await expect(status).toBeVisible({ timeout: 5_000 })
    await expect(status).toContainText(/Retry to reconnect to Kitty/i)
    await expect(status).not.toContainText(/gateway offline/i)
    await assertNoHorizontalOverflow(page)
    await page.screenshot({ path: testInfo.outputPath('02-picker-failure.png'), fullPage: true })
    expect(providerDispatches).toEqual([])

    phase = 'recovery'
    await page.unroute('**/proxy/models/picker')
    await status.getByRole('button', { name: /retry/i }).click()
    await expect(composer()).toBeEnabled({ timeout: 15_000 })
    await assertNoHorizontalOverflow(page)
    await page.screenshot({ path: testInfo.outputPath('03-recovered.png'), fullPage: true })
    await page.reload()
    await openChat()
    await expect(composer()).toBeEnabled({ timeout: 12_000 })

    phase = 'direct-provider'
    await page.route('**/proxy/api/providers', route => json(route, {
      active: 'openrouter', order: ['openrouter'], warnings: [], config_path: 'acceptance',
      providers: [{ name: 'openrouter', configured: true, disabled: false }],
    }))
    await page.route('**/proxy/api/models', route => json(route, { error: 'acceptance injected discovery outage' }, 503))
    await page.route('**/proxy/runtime/manifest**', route => json(route, { error: 'acceptance injected runtime outage' }, 503))
    await page.reload()
    await openChat()
    await expect(composer()).toBeEnabled({ timeout: 12_000 })
    await expect(page.getByRole('button', { name: 'Model: Daily Kitty' })).toBeEnabled()
    await expect(page.getByRole('status').filter({ hasText: /models temporarily unavailable|Model details unavailable|No live curated models/i })).toHaveCount(0)
    expect((await page.locator('body').innerText()).toLowerCase()).not.toContain('openrouter')
    await assertNoHorizontalOverflow(page)
    await page.screenshot({ path: testInfo.outputPath('04-direct-provider.png'), fullPage: true })
    expect(providerDispatches).toEqual([])
    await page.unroute('**/proxy/api/providers')
    await page.unroute('**/proxy/api/models')
    await page.unroute('**/proxy/runtime/manifest**')

    phase = 'disabled-provider'
    await page.route('**/proxy/api/providers', route => json(route, {
      active: 'agentrouter', order: [], warnings: ['selected provider is disabled by environment'], config_path: 'acceptance',
      providers: [{ name: 'agentrouter', configured: true, disabled: true }],
    }))
    await page.reload()
    await openChat()
    await expect(composer()).toBeDisabled({ timeout: 12_000 })
    const blocked = page.getByRole('status').first()
    await expect(blocked).toBeVisible({ timeout: 5_000 })
    await expect(blocked).toContainText(/Retry to reconnect to Kitty/i)
    expect((await page.locator('body').innerText()).toLowerCase()).not.toContain('agentrouter')
    await assertNoHorizontalOverflow(page)
    await page.screenshot({ path: testInfo.outputPath('05-disabled-provider.png'), fullPage: true })
    expect(providerDispatches).toEqual([])
    expect(controlPlaneWrites).toEqual([])
    expect(unexpectedPaths).toEqual([])
    const unexpectedBrowserErrors = browserErrors.filter(text =>
      !((text.startsWith('picker-failure:') || text.startsWith('direct-provider:')) && /503|Failed to load resource/i.test(text))
    )
