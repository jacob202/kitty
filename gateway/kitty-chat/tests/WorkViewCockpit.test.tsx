...
describe('WorkView recovery cockpit', () => {
  ...
  it('keeps a failed Work request editable and offers an inline retry', async () => {
    ...
    expect(screen.getByRole('alert')).toHaveTextContent('may use credits')
  ...
  it('offers an explicit one-proposal alternate-model recovery without replacing the saved route', async () => {
    ...
    expect(await screen.findByRole('button', { name: 'Try saved provider route (may use credits)' })).toBeInTheDocument()
    expect(request).toHaveValue('Fix the launch bug.')
    fireEvent.click(screen.getByRole('button', { name: 'Try saved provider route (may use credits)' }))
  ...
  it('uses one pending-only Work recovery key instead of the reload-reset proposal counter', async () => {
    window.localStorage.setItem('kitty.builder-proposal.work-builder-request.1', 'conv-old-job')
    const mutateAsync = vi.fn().mockResolvedValue({
      ok: true,
      task: { objective: 'Create the new proof', instructions: 'Create it.', allowed_paths: ['new-proof.txt'] },
    })
    useCompileBuilderProposal.mockReturnValue({ mutateAsync, isPending: false })
    render(<WorkView isMobile={false} />)

    fireEvent.change(screen.getByRole('textbox', { name: 'Ask Builder for work' }), { target: { value: 'Create the new proof.' } })
    fireEvent.click(screen.getByRole('button', { name: 'Prepare Builder proposal' }))

    const card = await screen.findByTestId('work-builder-proposal')
    expect(card).toHaveTextContent('Create the new proof')
    expect(card).toHaveAttribute('data-recovery-storage-key', 'kitty.builder-proposal.work.pending')
    expect(card).toHaveAttribute('data-persist-resolved-mission', 'false')
  })

  it('reattaches an ambiguous pending Work approval after reload without compiling a new proposal', async () => {
    window.localStorage.setItem('kitty.builder-proposal.work.pending', JSON.stringify({
      version: 1,
      state: 'pending',
