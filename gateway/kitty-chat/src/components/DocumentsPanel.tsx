...
const STATUS_COLORS: Record<string, string> = { success: 'var(--c-green)', skipped: 'var(--c-yellow)', failed: 'var(--c-red)', pending: 'var(--c-blue)' }
  ...
  return (
    <div style={{ display: 'grid', gap: 26, alignContent: 'start', minWidth: 0 }}>
      <section aria-labelledby="library-search-heading" style={subsectionStyle}>
        ...
        {submitted && searchQuery.isError && <p style={{ ...mutedStyle, color: 'var(--c-red)' }}>Search failed — {searchQuery.error instanceof Error ? searchQuery.error.message : 'gateway error'}</p>}
      ...
      <section aria-labelledby="library-add-heading" style={subsectionStyle}>
        ...
        {ingest.isError && <p role="status" style={{ ...mutedStyle, color: 'var(--c-red)' }}>Ingest failed — {ingest.error instanceof Error ? ingest.error.message : 'gateway error'}</p>}
        {ingest.data && (
          <div style={statusResultStyle}>
            <strong style={{ color: STATUS_COLORS[ingest.data.status] ?? 'var(--ink)' }}>{ingest.data.status === 'success' ? 'indexed' : ingest.data.status}</strong>
        ...
        <button
          ...
          style={{ ...dropZoneStyle, borderColor: dragOver ? 'var(--primary)' : 'var(--line)', background: dragOver ? 'var(--primary-fade)' : 'var(--surface)' }}
        >
        ...
        {upload.isError && <p role="status" style={{ ...mutedStyle, color: 'var(--c-red)' }}>Upload failed — {upload.error instanceof Error ? upload.error.message : 'gateway error'}</p>}
        {upload.data && <p style={{ ...mutedStyle, color: STATUS_COLORS[upload.data.status] ?? 'var(--ink-2)' }}>{upload.data.status}: {upload.data.message} — it appears under indexed sources when processing finishes.</p>}
      </section>
      ...
      <section aria-labelledby="library-sources-heading" style={subsectionStyle}>
        ...
        {sourcesQuery.isError && <p role="status" style={{ ...mutedStyle, color: 'var(--c-red)' }}>Couldn&apos;t read the knowledge index — {sourcesQuery.error instanceof Error ? sourcesQuery.error.message : 'gateway error'}.</p>}
...
const subsectionStyle: CSSProperties = { display: 'grid', gap: 12, minWidth: 0, paddingTop: 18, borderTop: '1px solid var(--line)' }
const subsectionTitleStyle: CSSProperties = { margin: 0, fontFamily: 'var(--font-display)', fontSize: 17, fontWeight: 700, color: 'var(--ink)' }
const subtitleStyle: CSSProperties = { fontSize: 13, lineHeight: 1.5, color: 'var(--ink-2)', margin: '4px 0 0' }
const inputStyle: CSSProperties = { width: '100%', minWidth: 0, minHeight: 44, background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: 'var(--r-control)', padding: '10px 12px', fontFamily: 'var(--font-body)', fontSize: 14, color: 'var(--ink)', outline: 'none' }
const primaryButtonStyle: CSSProperties = { minHeight: 44, padding: '9px 16px', background: 'var(--primary)', color: 'var(--on-primary)', border: 'none', borderRadius: 'var(--r-control)', fontFamily: 'var(--font-body)', fontSize: 14, fontWeight: 650, cursor: 'pointer' }
const secondaryButtonStyle: CSSProperties = { minHeight: 44, padding: '8px 13px', border: '1px solid var(--line)', borderRadius: 'var(--r-control)', background: 'var(--surface)', color: 'var(--ink)', fontWeight: 600, flexShrink: 0 }
const mutedStyle: CSSProperties = { margin: 0, fontSize: 13, color: 'var(--ink-2)', lineHeight: 1.55, overflowWrap: 'anywhere' }
const resultStyle: CSSProperties = { padding: '12px 0', borderBottom: '1px solid var(--line)', display: 'grid', gap: 5 }
const resultSourceStyle: CSSProperties = { fontSize: 13, color: 'var(--primary)', overflowWrap: 'anywhere' }
const resultTextStyle: CSSProperties = { margin: 0, fontSize: 13, lineHeight: 1.55, color: 'var(--ink)' }
const metaStyle: CSSProperties = { fontFamily: 'var(--font-mono)', fontSize: 10, color: 'var(--ink-2)', overflowWrap: 'anywhere' }
const statusResultStyle: CSSProperties = { padding: '10px 12px', background: 'var(--surface-2)', borderRadius: 'var(--r-control)', display: 'grid', gap: 3, fontSize: 12, color: 'var(--ink-2)' }
...
const dropZoneStyle: CSSProperties = { width: '100%', minHeight: 44, border: '1px dashed var(--line)', borderRadius: 'var(--r-control)', padding: '12px 14px', fontFamily: 'var(--font-body)', fontSize: 13, color: 'var(--ink-2)', textAlign: 'center', cursor: 'pointer', lineHeight: 1.45 }
const sourceRowStyle: CSSProperties = { padding: '12px 0', borderBottom: '1px solid var(--line)', display: 'grid', gap: 5, minWidth: 0 }
const sourceNameStyle: CSSProperties = { fontSize: 14, color: 'var(--ink)', overflowWrap: 'anywhere' }
const topicStyle: CSSProperties = { margin: '3px 0 0', fontSize: 12, color: 'var(--ink-2)', lineHeight: 1.5 }
const sourceMetaStyle: CSSProperties = { display: 'flex', gap: '4px 10px', flexWrap: 'wrap', color: 'var(--ink-2)', fontSize: 11 }
