...
def list_plugins(enabled_only: bool = False) -> list[dict]:
    ...
    for name, definition in _registry.items():
        ...
        enabled = (
            user_setting if user_setting is not None else definition.get("default_enabled", True)
        )
...
def _save_db_settings(settings: dict) -> None:
    with kitty_db.connect(PLUGIN_DB_FILE) as conn:
        conn.execute("DELETE FROM plugin_settings")
        rows = [
            (str(name), 1 if bool(enabled) else 0) for name, enabled in sorted(settings.items())
        ]
        conn.executemany(
...
def _migrate_legacy_settings_once() -> None:
    ...
    with kitty_db.connect(PLUGIN_DB_FILE) as conn:
        ...
        if flag is not None:
            return
        db_count = conn.execute("SELECT COUNT(*) FROM plugin_settings").fetchone()[0]
        if db_count > 0:
