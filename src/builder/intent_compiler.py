...
def compile_intent(project_root: str | Path, raw_input: str) -> BuilderBrief:
    """
    Compile a raw user request into a structured BuilderBrief using high-reasoning LLM analysis.
    """
    ...
    if not raw:
        return _vague_brief(raw, "No request text was provided.")

    prompt = f"""You are the Kitty Intent Compiler. Your task is to analyze a raw user request and produce a structured engineering brief.

PROJECT ROOT: {root}
USER REQUEST: "{raw}"

TASK:
1.  **Normalize the Goal**: Define exactly what needs to be done in 1 sentence.
2.  **Identify Success Criteria**: List 2-3 technical milestones for completion.
3.  **Identify Risks & Ambiguities**: What could go wrong? What is unclear?
4.  **Define the 'Boring Path'**: What is the most conservative, low-risk, and surgical way to handle this task? Prioritize reliability and state integrity over speed or broad refactoring.
5.  **Choose Execution Mode**: scout (research), single_worker (simple fix), parallel_workers (complex feature), or human_question (clarification needed).

Respond in JSON format matching this schema:
{{
  "normalized_goal": "...",
  "success_criteria": ["...", "..."],
  "boring_path": "The conservative approach is...",
  "risks": ["...", "..."],
  "ambiguities": ["...", "..."],
  "mode": "scout|single_worker|parallel_workers|human_question",
  "confidence": 0.0-1.0
}}
"""

    try:
        # Use high-reasoning Claude 3.7 for intent compilation
        payload = {
        ...
        brief = BuilderBrief(
            ...
            validation_commands=_validation_commands(root, raw.lower()),
            allowed_files=[],
            forbidden_files=[],
            assumptions=[data.get("boring_path", "Maintain existing architectural patterns.")],
            ambiguities=data.get("ambiguities", []),
            blocking_question="" if data.get("mode") != "human_question" else data.get("ambiguities", ["Please clarify"])[0],
            context_targets=_context_targets(root, raw.lower()),
            risk_level="high" if data.get("confidence", 1.0) < 0.5 else "medium",
            recommended_execution_mode=data.get("mode", "single_worker"),
            handoff_prompt=f"Execute this scoped goal: {data.get('normalized_goal')}",
            confidence=data.get("confidence", 0.8),
        )
        return _with_packet(brief, stage="intent_compiled")

    except Exception as e:
        # Fallback to legacy rule-based compilation if LLM fails
        return _legacy_compile_intent(root, raw)


def _vague_brief(raw: str, reason: str) -> BuilderBrief:
    return BuilderBrief(
        raw_input=raw,
...
def _legacy_compile_intent(root: Path, raw: str) -> BuilderBrief:
    lower = raw.lower()
