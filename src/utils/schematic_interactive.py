...
def example():
    """Example demonstrating ComponentHotspot and HotspotManager usage."""

    # Create hotspot manager
    manager = HotspotManager(allow_multi_select=False)
    ...
    components = [
        ComponentHotspot(
            ...
            x=100,
            y=150,
            metadata={"value": "10kΩ", "package": "0805", "tolerance": "1%"},
        ),
        ComponentHotspot(
            designator="C203",
            ...
            x=200,
            y=150,
            metadata={"value": "100nF", "package": "0603", "voltage": "50V"},
        ),
        ComponentHotspot(
            designator="U1",
            component_type="IC",
            ...
            width=40,
            height=30,
            metadata={"part_number": "LM358", "package": "SOIC-8"},
        ),
    ]

    # Add hotspots with tooltips and event handlers
    for comp in components:
        comp.add_tooltip(
            content=f"<strong>{comp.designator}</strong><br>{comp.component_type}", position="top"
        )
        comp.on_click(lambda h: print(f"Clicked: {h.designator}"))
        comp.on_select(lambda h, s: print(f"{'Selected' if s else 'Deselected'}: {h.designator}"))
        manager.add_hotspot(comp)

    # Generate outputs
    print("=== JavaScript Handlers ===")
    print(manager.generate_js_handlers()[:500] + "...\n")

    print("=== JSON Export ===")
    print(manager.to_json()[:500] + "...\n")

    print("=== CSS Styles ===")
    print(manager.generate_css()[:500] + "...\n")

    # Save files
    manager.save_js_file("output/schematic_hotspots.js")
    manager.save_css_file("output/schematic_hotspots.css")
    manager.save_json_file("output/schematic_hotspots.json")
