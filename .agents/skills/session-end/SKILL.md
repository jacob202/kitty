...
The KB is not self-improving merely because agents write to it. Record whether it
was consulted, useful, stale, costly, and associated with a verified outcome.
...
Read [references/receipt-schema.md](references/receipt-schema.md) for the full
field set, the field rules (never estimate `null`-able metrics; `accepted`
requires the step 3A review plus independent evidence; consulted/used/stale
subset rules; session_id identity rules; hash-chained idempotent storage at
`~/kb/metrics/kb-effectiveness.jsonl`), and the summary/report commands.

Do not claim the KB saves tokens or improves code until the report has sufficient
accepted results, complete measurements, and an independently reviewed
comparison. Report all evidence gaps.
