---
name: isa
description: "Owns the Ideal State Artifact — the primitive holding the articulated ideal state of any thing (project, task, app, library, infrastructure, session) as a hard-to-vary explanation. One document articulates the ideal state, drives the build, verifies it, and records evolving understanding. Five workflows — Scaffold, Interview, CheckCompleteness, Reconcile, Seed. Twelve-section body order is locked (Problem, Vision, Out of Scope, Principles, Constraints, Goal, Criteria, Test Strategy, Features, Decisions, Changelog, Verification). Invoked automatically at OBSERVE for non-trivial tasks and directly by the user. USE WHEN any prompt mentions ideal state, ISA, ISC, ideal state criteria, ideal state artifact, project specification, hill-climbing, articulating done, scaffolding an ISA, interviewing for an ISA, checking ISA completeness, reconciling a feature file, or seeding an ISA from a repository. NOT FOR creating new skills (use CreateSkill) or running the Algorithm itself."
effort: medium
---
