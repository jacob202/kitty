...
<identity> --unread --direct-only --json`; with Agent Room MCP use
`room_inbox(unread_only=True, direct_only=True)`. When a handoff or current
assignment supplies a durable locator, load that exact
