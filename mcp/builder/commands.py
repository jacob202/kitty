...
def _approval_nonce(
    *, manifest_sha: str, base_sha: str, refs: dict[str, str]
) -> str:
    payload = json.dumps(
        {
            "schema": 1,
...
def _bind_artifacts(
    ...
    *,
    ...
    # Reads prove both path + commit exist without exposing arbitrary filesystem access.
    ...
    try:
        ...
        if current_base != expected_base_sha:
            return receipt(
                ...
                next_action="Rebase/review the design and plan against the current main branch.",
            )
        ...
        return receipt(
            ...
            ok=True,
            state="prepared",
            summary="Mission is validated and bound to exact design/plan/base revisions.",
    ...
    *,
    ...
    try:
        ...
        return receipt(
            ...
            next_action="Use work_status/resume_context for durable progress rather than launcher narration.",
        ...
        return receipt(
            ...
            error=f"{type(exc).__name__}: {exc}",
            next_action="Inspect Builder status before retrying; do not launch a second owner blindly.",
        )
    ...
    *,
    ...
    if not confirmed:
        return receipt(
            ...
            error="publication pushes a branch/creates or updates a PR and requires explicit confirmation",
            next_action="Confirm publication explicitly after reviewing the task result/evidence.",
        )
    try:
        _validate_id(task_id, label="task_id")
        ...
        return receipt(
            ...
            pr=result.evidence,
            next_action="Review the PR and its required checks; merging remains a separate human action.",
        )
    except Exception as exc:
        return receipt(
            "publication_prepare",
