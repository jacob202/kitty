...
async def test_search_all_returns_all_keys():
    """search_all should always return the four canonical keys."""
    # Mocking external store fetchers to return empty lists
    with patch("gateway.memory_graph._fetch_memory", return_value=[]), \
         patch("gateway.memory_graph._fetch_knowledge", return_value=[]), \
         patch("gateway.memory_graph._fetch_journal", return_value=[]), \
         patch("gateway.memory_graph._fetch_traces", return_value=[]):
        
        results = await search_all("test query")
        assert set(results.keys()) == {"memory", "knowledge", "journal", "traces"}
...
async def test_failure_isolation():
    ...
    with patch("gateway.memory_graph._fetch_memory", side_effect=RuntimeError("Store down")), \
         patch("gateway.memory_graph._fetch_knowledge", return_value=[{"text": "found it"}]), \
         patch("gateway.memory_graph._fetch_journal", return_value=[]), \
         patch("gateway.memory_graph._fetch_traces", return_value=[]):
...
async def test_real_journal_fetch_smoke(tmp_path, monkeypatch):
    """Test the real _fetch_journal logic with a temporary file."""
    journal_file = tmp_path / "journal_entries.jsonl"
    monkeypatch.setattr("gateway.memory_graph.JOURNAL_LOG", journal_file)
    ...
    from gateway.memory_graph import _fetch_journal
    results = _fetch_journal("quick dog")
...
async def test_real_trace_fetch_smoke(tmp_path, monkeypatch):
    ...
    with open(trace_file, "w") as f:
        f.write(json.dumps({"user_request": "fix the hvac", "timestamp": now, "domain_classified": "repair"}) + "\n")
        f.write(json.dumps({"user_request": "stale request", "timestamp": now - 10*86400, "domain_classified": "old"}) + "\n")
