...
def test_mcp_up_reuses_shared_checkout_venv_from_linked_worktree(
    launcher_repo, tmp_path: Path
):
    root, env = launcher_repo
    shared = tmp_path / "shared-kitty"
    shared_python = shared / "venv" / "bin" / "python"
    shared_python.parent.mkdir(parents=True)
    shutil.copy2(root / "venv" / "bin" / "python", shared_python)
    ...
    bad_fallback = fake_bin / "python3.12"
    bad_fallback.write_text("#!/usr/bin/env bash\nexit 97\n", encoding="utf-8")
    bad_fallback.chmod(0o755)
    env["TEST_SHARED_ROOT"] = str(shared)
