"""Tests for builder success-criteria helpers (ISA-lite derive/check)."""

from unittest.mock import patch

from gateway.builder import isc as sc
...
def test_derive_empty_goal_returns_empty():
    assert sc.derive_criteria("   ") == []


def test_derive_llm_failure_returns_empty():
    with patch("gateway.builder.isc.llm_client.chat", side_effect=RuntimeError("boom")):
...
def test_check_parses_json_array():
    ...
    with patch("gateway.builder.isc.llm_client.chat", return_value=resp):
...
def test_check_no_criteria_returns_empty():
    assert sc.check_criteria("goal", [], "evidence") == []


def test_check_llm_failure_is_neutral():
    with patch("gateway.builder.isc.llm_client.chat", side_effect=RuntimeError("x")):
...
def test_format_block_for_strings_and_results():
    assert sc.format_criteria_block([]) == ""
    assert "- [ ] do thing" in sc.format_criteria_block(["do thing"])
    block = sc.format_criteria_block([{"criterion": "x", "passed": True, "note": "good"}])
    assert "- [x] x — good" in block


def test_all_passed():
    assert sc.all_criteria_passed([]) is False
    assert sc.all_criteria_passed([{"passed": True}, {"passed": True}]) is True
    assert sc.all_criteria_passed([{"passed": True}, {"passed": False}]) is False
