...
def test_forbidden_changes_no_conflict():
    findings = validate_scope(
        _packet(
            allowed_paths=["gateway/foo.py"],
            forbidden_changes=["docs/adr/0001-x.md"],
        )
    )
    assert not any(f.category == "scope_conflict" for f in findings)


def test_forbidden_changes_overlap_is_scope_conflict():
    findings = validate_scope(
        _packet(
            allowed_paths=["gateway/foo.py", "docs/adr/0001-x.md"],
            forbidden_changes=["docs/adr/0001-x.md"],
        )
    )
    assert any(
        f.category == "scope_conflict" and f.field == "forbidden_changes"
        for f in findings
    )


def test_forbidden_changes_bad_path_is_unbounded():
    findings = validate_scope(
        _packet(forbidden_changes=["../secrets"])
    )
    assert any(
        f.category == "unbounded_scope" and f.field == "forbidden_changes"
        for f in findings
    )
