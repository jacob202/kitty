...
def test_get_work_returns_projected_snapshot(client: TestClient, monkeypatch: pytest.MonkeyPatch):
    monkeypatch.setattr(
        work_route,
                    ...
                    "packets": [
                        {
                            ...
                            "attempt_history": [{"validation": {"status": "passed"}, "review": {"verdict": "approved"}}],
                            "publication": None,
...
def test_get_work_returns_503_with_concrete_reason_when_snapshot_fails(
    client: TestClient,
