...
async def test_unified_context_aggregation():
    ...
    with patch("gateway.memory_graph._fetch_memory", new_callable=AsyncMock) as m_mem, \
         patch("gateway.memory_graph._fetch_knowledge", new_callable=AsyncMock) as m_kn, \
         patch("gateway.memory_graph._fetch_journal", return_value=[{"entry": "journal test", "ts": 123}]), \
         patch("gateway.memory_graph._fetch_traces", return_value=[{"user_request": "trace test", "domain_classified": "soul"}]):
        
        m_mem.return_value = [{"memory": "memory test"}]
        m_kn.return_value = [{"source": "kn_src", "doc_type": "manual", "text": "knowledge test"}]
...
async def test_search_all_structure():
    with patch("gateway.memory_graph._fetch_memory", new_callable=AsyncMock) as m_mem, \
         patch("gateway.memory_graph._fetch_knowledge", new_callable=AsyncMock) as m_kn, \
         patch("gateway.memory_graph._fetch_journal", return_value=[{"entry": "journal test"}]), \
         patch("gateway.memory_graph._fetch_traces", return_value=[{"user_request": "trace test"}]):
