...
class TestSaveDreamInsights:
    def test_one_card_per_line(self, isolated_file: Path) -> None:
        summary = (
    ...
    def test_classifies_warning_kind(self, isolated_file: Path) -> None:
        dream_insights.save_dream_insights("Consolidation error: boom")
        cards = json.loads(isolated_file.read_text(encoding="utf-8"))
        assert cards[0]["kind"] == "warning"

    def test_classifies_reflection_kind(self, isolated_file: Path) -> None:
        dream_insights.save_dream_insights("Weekly mirror refreshed: ...")
        cards = json.loads(isolated_file.read_text(encoding="utf-8"))
        assert cards[0]["kind"] == "reflection"

    def test_cards_have_required_fields(self, isolated_file: Path) -> None:
    ...
    def test_empty_summary_yields_no_cards(self, isolated_file: Path) -> None:
...
class TestLoadDreamInsights:
    ...
    def test_normalizes_iso_timestamp(self, isolated_file: Path) -> None:
    ...
    def test_respects_limit(self, isolated_file: Path) -> None:
        isolated_file.write_text(
            json.dumps(
                [
                    {
...
class TestDismiss:
    def test_removes_named_card(self, isolated_file: Path) -> None:
        isolated_file.write_text(
            json.dumps(
                [
                    {
    ...
    def test_returns_false_when_missing(self, isolated_file: Path) -> None:
...
class TestStatus:
    def test_returns_never_run_shape_when_empty(self, isolated_file: Path) -> None:
        ...
        with mock.patch.object(
            ...
            "get_last_run_info",
            return_value={"last_run": None, "never": True},
        ):
