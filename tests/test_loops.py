...
class TestSeed:
    def test_first_init_seeds_three_loops(self, isolated_db: Path) -> None:
        rows = loops.list_loops()
        ids = {row["loop_id"] for row in rows}
        assert ids == {"daily-brief", "search-index", "memory-consolidation"}

    def test_seed_preserves_statuses(self, isolated_db: Path) -> None:
        rows = loops.list_loops()
    ...
    def test_seed_preserves_intervals(self, isolated_db: Path) -> None:
        rows = loops.list_loops()
        by_id = {r["loop_id"]: r for r in rows}
    ...
    def test_second_init_does_not_duplicate(self, isolated_db: Path) -> None:
...
class TestCreateLoop:
    ...
    def test_collisions_get_a_suffix(self, isolated_db: Path) -> None:
...
class TestToggle:
    ...
    def test_updates_timestamp(self, isolated_db: Path) -> None:
        ...
        after_ts = next(r for r in loops.list_loops() if r["loop_id"] == "search-index")[
            "updated_at"
        ]
