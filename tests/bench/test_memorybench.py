...
def test_substring_false_positive_is_not_regressed(tmp_path):
    cases = {case["id"]: case for case in load_cases(FIXTURES)}
    result = run_case(
        cases["substring_false_positive_001"],
        tmp_path / "substring.db",
    )
    # The stranded branch's original fixture documented "automobile" being
    # returned for "mobile phone" as a known baseline weakness. Token-aware
    # MemoryWeave search (PR #575) removed that substring false positive, so
    # current main must not regress it.
