...
class TestLogPerfStat:
    ...
    def test_raises_on_write_failure(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
class TestGetPerfStats:
    ...
    def test_rejects_non_positive_window(self, isolated_log: Path) -> None:
        with pytest.raises(ValueError, match="window_hours"):
            perf.get_perf_stats(window_hours=0)
        with pytest.raises(ValueError, match="window_hours"):
            perf.get_perf_stats(window_hours=-1)  # type: ignore[arg-type]

    def test_reports_active_schedule_count(self, isolated_log: Path) -> None:
...
class TestGetRecentStats:
    ...
    def test_returns_newest_first(self, isolated_log: Path) -> None:
        now = perf.time.time()
        for i in range(5):
