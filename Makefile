...
# Hygiene jobs from the Tests workflow's `hygiene` run that `make ci` used to
# omit; parity keeps a green local run from shipping code CI would reject.
# Same commands and thresholds as .github/workflows/tests.yml.
vulture:
